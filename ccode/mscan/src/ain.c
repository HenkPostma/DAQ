...
//        prepare_cmd(dev, options.subdevice, options.n_scan, options.n_chan, 1, cmd);
         
        prepare_cmd(dev, options.subdevice, options.n_scan, options.n_chan, period, cmd);
        ...
        /* Found that cmd->convert_arg is a good way to find out what the sample rate is 
         * So we'll use that to set the period
         */ 
        printf("detected period %i ns", cmd->convert_arg);
        deltat = cmd->convert_arg;
        /* end setting period */
        ...
        while(1){
                ...
                if(ret < 0){
                        ...
                        for(i = 0; i < ret / bytes_per_sample; i++){
                                ...
                                timebase0 = timebase0+deltat;
         	                    if (col == 0) {
                                   fprintf(file, "%f ", timebase0);
                                }
                                fprintf(file, "%i ", raw);
...
void print_datum(lsampl_t raw, int channel_index, short physical) {
        ...
        if(!physical) {
                printf("%d ",raw);
//                printf("%#8.6g ",(raw/3276.6-10)/0.953*1.0003841+0.0045897);
        } else {
