...
secs = 0.01 # used to stop scan after "secs" seconds
packetSize = 512
...
def prepare_cmd(dev, subdev, C):
    #global cmd
    C.subdev = subdev
#    dev2 = C.comedi_open('/dev/comedi0')
#    C.flags = C.comedi_get_subdevice_flags(dev2, subdev)
    C.flags = 32616448
...
#dev2 = C.comedi_open('/dev/comedi0')
cmd.flags = c.comedi_get_subdevice_flags(dev, cmd.subdev)
#C.flags = 32616448
...
cmd.convert_arg = 1 # int(1e9/freq/16)
cmd.scan_end_src = c.TRIG_COUNT
cmd.scan_end_arg = nchans
cmd.stop_src = c.TRIG_COUNT
cmd.stop_arg = nchans*size
cmd.chanlist = mylist
cmd.chanlist_len = nchans
...
# this is old stuff, new read routine below...
if False:
    while flag:
